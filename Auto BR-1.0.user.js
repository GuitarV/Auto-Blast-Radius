...
// @updateURL    https://drive.corp.amazon.com/documents/BJS%20DCEO/Auto%20BR-1.0.user.js
// @downloadURL  https://drive.corp.amazon.com/documents/BJS%20DCEO/Auto%20BR-1.0.user.js
// ==/UserScript==
...
(function() {
    ...
    function getFilterOptions() {
        ...
        return [
            { label: 'Position Room', column: 'Position Room' },
            {
                label: 'Position',
                column: 'Position',
                isPosition: true
    ...
    async function fetchPositionInfo(site) {
        const positionInfoUrl = `https://cloudforge-build.amazon.com/datacenters/${site}/equipments/floorplans/positions.json`;
        const networkConnectivityUrl = `https://cloudforge-build.amazon.com/datacenters/${site}/floorplans/network_connectivity.json`;
        const euclidUrl = `https://aha.bjs.aws-border.cn/blast-radius/api/get-euclid-bricks-for-site/${site}`;

        try {
            // 首先获取基本位置信息和网络连接信息
            const [positionResponse, networkResponse] = await Promise.all([
                makeRequest(positionInfoUrl, 'GET'),
                makeRequest(networkConnectivityUrl, 'GET')
            ]);

            let positionData;
            try {
                positionData = JSON.parse(positionResponse.responseText);
            ...
            let networkData;
            try {
                networkData = JSON.parse(networkResponse.responseText);
            } catch (e) {
            ...
            // 尝试获取 Euclid 数据，但如果失败则继续执行
            let hasEuclidAccess = false;
            try {
                const euclidResponse = await makeRequest(euclidUrl, 'GET');
                const euclidData = JSON.parse(euclidResponse.responseText);
                hasEuclidAccess = true;

                if (euclidData && typeof euclidData === 'object') {
                ...
                console.warn('Failed to fetch Euclid data:', error);
                // Euclid 数据获取失败，继续使用空的映射
            }
    ...
    async function updateDisplay(filters) {
        ...
        try {
            ...
            // 处理位置数据
            const positions = {};
            const usedCircuits = new Set(); // 用于存储所有被使用的 circuit

            // 首先遍历数据来收集所有被使用的 circuit
            EXCEL_DATA.forEach(row => {
                try {
                    // 确保数据存在且可以转换为字符串
                    const positionRoom = row['Position Room'];
                    const position = row['Position'];

                    // 检查值是否存在且可以转换为字符串
                    if (positionRoom && position &&
                        String(positionRoom).trim() !== '' &&
                        String(position).trim() !== '') {

                        // 确保 Circuit Name 存在
                        if (row['Circuit Name']) {
                            usedCircuits.add(row['Circuit Name']);
                        }
                    }
                } catch (error) {
            ...
            EXCEL_DATA.forEach(row => {
                if (!usedCircuits.has(row['Circuit Name'])) {
                    return;
                }
                ...
                if (!positions[positionKey]) {
                    ...
                    positions[positionKey] = {
                        site: row['Position Site'],
                        room: row['Position Room'],
                ...
                const powerChain = {
                    ...
                    usb: row['USB'],
                    powerFeed: row['Power Feed'],
                    routingInfo: row.routingInfo || { transformer: 'Unknown', utility: 'Unknown' }
                };
            ...
            // 计算统计信息
            Object.entries(positions).forEach(([positionKey, position]) => {
                if (position.status === 'deployed') {
                    const type = (position.type || 'unknown').toUpperCase();
                    const posInfo = positionMap.get(positionKey);
                    const isEuclid = posInfo?.is_brick === true;

                    // 所有机架都按原有类型统计，不再分离 Euclid
                    ...
                    // 记录每种类型的 Euclid 数量
                    if (isEuclid && !stats.detailedStats[type].euclidCount) {
                    ...
                    if (type === 'NETWORK') {
                        if (!hasDualPower) {
                            ...
                            if (remainingPrimary === 0 && expected.primary > 0) {
                                ...
                                if (isEuclid) {
                                    if (!stats.detailedStats[type].euclidCount) {
                                        stats.detailedStats[type].euclidCount = {
                                            'Lost Primary': 0,
                                            'Lost Secondary': 0,
                                            'Partial Power Loss': 0,
                ...
                <div class="stats-container">
                                    ...
                                    ${['Lost Primary', 'Lost Secondary', 'Partial Power Loss', 'Complete Power Loss'].map(metric => {
                                        return `
                                            ...
                                            return generateStatsCell(
                                                ...
                                                <td class="stats-cell">${
                                                    activeRackTypes.reduce((sum, type) =>
                    ...
                    <div class="export-button-container">
                        <button id="exportStatsBtn" class="export-button">
                            <span class="export-icon">📋</span> Copy Table
                        </button>
                    </div>
                </div>`;

            const positionsToShow = new Set(
                filteredData.map(row => `${row['Position Room']}-${row['Position']}`)
            );
            ...
            .map(([key, position]) => {
                const positionKey = `${position.room}-${position.position}`;
                const posInfo = positionMap.get(positionKey);
                const hasEuclidAccess = posInfo?.hasEuclidAccess || false;
                const isEuclid = posInfo?.is_brick === true;
                    ...
                    <div class="topo-item ${isEuclid ? 'euclid-brick' : ''}">
                        <div class="topo-item-header">
                            <div class="position-info">
                                <span class="status-indicator status-${posInfo?.status.toLowerCase() || 'unknown'}"></span>
                                <span class="position-id">${position.room} ${position.position}</span>
                                ${posInfo?.status === 'deployed' ?
                    `<span class="rack-type">${posInfo?.type || 'Unknown'}</span>` :
                ''}
                                ${posInfo?.power_redundancy ?
                    `<span class="power-redundancy">(${posInfo.power_redundancy})</span>` :
                ''}
                                ${isEuclid ?
                    `<span class="euclid-tag">
                                        ${posInfo.deployed_asset_id ?
                    `<a href="https://aha.bjs.aws-border.cn/host-monitoring/euclid/${posInfo.deployed_asset_id}"
                                                target="_blank"
                ...
                ''}
                            ...
                            <div class="position-tags">
                                <span class="filter-tag status-tag-${posInfo?.status.toLowerCase() || 'unknown'}">${posInfo?.status || 'Unknown'}</span>
                                <span class="filter-tag">Circuits: ${position.powerChains.length}</span>
                                ${posInfo?.power_kva ? `<span class="filter-tag">Power: ${posInfo.power_kva} kVA</span>` : ''}
                            </div>
                        </div>
                        <div class="topo-item-content">
                            ${position.powerChains.map(chain => `
                                <div class="power-chain power-chain-${chain.powerFeed.toLowerCase()}">
                                    <div class="chain-header">Power Feed: ${chain.powerFeed}</div>
                                    <div class="chain-path">
                                        ...
                                        <div class="chain-item">
                                            ...
                                            <div class="chain-value">${chain.routingInfo?.transformer || 'Unknown'}</div>
                                        </div>
                                        <div class="chain-arrow">→</div>
                                        <div class="chain-item">
                                            <div class="chain-label">Utility</div>
                                            <div class="chain-value">${chain.routingInfo?.utility || 'Unknown'}</div>
                                        </div>
                                    </div>
                                </div>
                            `).join('')}
            ...
            topoView.innerHTML = statsHtml + positionsHtml;
    ...
    async function copyToClipboard(text) {
        try {
            ...
            btn.innerHTML = '<span class="export-icon">✓</span> Copied!';
    ...
    function initializeFilters(filtersContainer) {
        ...
        filters.forEach(filter => {
            ...
            if (filter.column === 'status') {
                ...
                const types = [...new Set(Array.from(positionMap.values())
                                          .map(info => info.type)
                                          .filter(Boolean)
                                         )];
                types.forEach(type => {
            ...
            } else if (filter.column === 'routingInfo.transformer') {
                // 为 transformer 添加选项
                const transformers = [...new Set(EXCEL_DATA
                                                 .map(item => item.routingInfo?.transformer)
                                                 .filter(Boolean))];
                transformers.forEach(transformer => {
                    select.append(new Option(transformer, transformer));
                });
            } else if (filter.column === 'routingInfo.utility') {
                // 为 utility 添加选项
                const utilities = [...new Set(EXCEL_DATA
                                              .map(item => item.routingInfo?.utility)
                                              .filter(Boolean))];
                utilities.forEach(utility => {
                    select.append(new Option(utility, utility));
                });
            } else if (filter.isPosition) {
                const positions = [...new Set(EXCEL_DATA
            ...
            cell.addEventListener('click', () => {
                try {
                    ...
                    if (cell.dataset.type && cell.dataset.metric) {
                        ...
                        const euclidPositions = positions.filter(position => {
                            ...
                            const matchingPosition = Object.entries(window.positions).find(([key, pos]) => {
                                const [room, pos_number] = key.split('-');
                                return pos.position === position && pos.type.toUpperCase() === type;
                            });
                        ...
                        modal.querySelector('.modal-title').textContent =
                            `${type} - ${metric} (${positions.length} positions${
                    ...
                    console.log('Cell data:', {
                        ...
                        positions: cell.dataset.positions
                    });
                }
            });
        });
...
.stats-cell.clickable {
    ...
    transition: background-color 0.2s;  /* 添加hover效果的平滑过渡 */
}
